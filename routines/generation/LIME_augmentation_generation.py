...
def main():
    ...
    for images, labels, names in loaded:
        ...
        for batch_id in range(img.shape[0]):
            transformed = img[batch_id]
            segments = segmentation_fn(img[batch_id])
            segments = torch.from_numpy(segments).unsqueeze(0\
                       ).to(args.device)
            n_interpret_features = len(segments.unique())
            # LIME Forwarding
            saliency = LIME.attribute(images[batch_id].unsqueeze(dim=0),
                       target=int(targets[batch_id]), 
                       feature_mask=segments, n_samples=40,
                       perturbations_per_eval=16, show_progress=False)
